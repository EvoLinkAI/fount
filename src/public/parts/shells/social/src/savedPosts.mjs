...
async function enrichPostRef(username, ref) {
	const entityHash = String(ref.entityHash || '').toLowerCase()
	...
	try {
		...
		else if (content?.text) preview = String(content.text).slice(0, 120)
...
export async function addSavedPost(username, postRef, folderId = null) {
	...
	const ref = {
		entityHash: String(postRef.entityHash).toLowerCase(),
...
export async function createSavedFolder(username, name) {
	...
	data.folders[folderId] = { name: String(name || 'Folder'), posts: [] }
	return saveSavedPosts(username, data)
}

/**
 * 【文件】profile/localized.mjs
 * 【职责】资料多语言切片规范化、locale 优先级与对外展示字段（name/avatar/links）解析。
 * 【原理】localesFromRequest 合并 Accept-Language 与用户偏好；resolveProfilePresentation 按 locale 链合并 localized；agent 走 agentResolve 取 part 元数据。
 * 【数据结构】ProfileLocaleSlice、localized map、ResolvedProfilePresentation、locales 字符串数组。
 * 【关联】被 profile.mjs、endpoints.mjs、groupSync 调用；依赖 personaPresentation、agentResolve、profileAvatarUrl。
 */
...
import { profileAvatarUrl } from './profile.mjs'
...
/**
 * @typedef {Partial<single_lang_info_t> & { links?: ProfileLink[] }} ProfileLocaleSlice
 */

/**
 * @typedef {single_lang_info_t & { links: ProfileLink[] }} ProfileInfoDefaults
 */

/**
 * @typedef {single_lang_info_t & { links: ProfileLink[] }} ResolvedProfilePresentation
 */
...
export function localesFromRequest(req, replicaUsername) {
	const raw = req.query?.locales
	/** @type {string[]} */
	let fromQuery = []
	if (Array.isArray(raw))
		fromQuery = raw.map(String)
	else if (raw)
		fromQuery = String(raw).split(',').map(s => s.trim()).filter(Boolean)
...
/**
 * @param {unknown} raw 链接项
 * @returns {ProfileLink | null} 规范化链接或 null
 */
...
/**
 * @param {Partial<single_lang_info_t>} li part info 切片
 * @returns {ProfileLink[]} 由 home_page / issue_page 推导的默认链接
 */
...
/**
 * @param {unknown} raw 原始 `localized` 字段
 * @returns {Record<string, ProfileLocaleSlice>} 规范化映射
 */
export function normalizeLocalizedMap(raw) {
	if (!raw) return {}
	/** @type {Record<string, ProfileLocaleSlice>} */
	const out = {}
...
/**
 * @param {Record<string, ProfileLocaleSlice>} localized 多语言切片
 * @param {string[]} locales 区域设置优先级
 * @returns {ProfileLocaleSlice | null} 最佳匹配切片
 */
...
/**
 * @param {Partial<single_lang_info_t>} li part `getInfo` 切片
 * @returns {ProfileInfoDefaults} 默认展示字段
 */
 ...
 * @returns {Promise<ProfileInfoDefaults>} 来自 part `getInfo` 的默认展示字段
 */
 ...
 * @param {ProfileInfoDefaults} infoDefaults part 默认
 * @returns {ResolvedProfilePresentation} 解析后的展示字段
 */
export function resolveProfilePresentation(stored, locales, infoDefaults) {
	...
	if (avatar && !avatar.startsWith('http') && isEntityHash128(stored?.entityHash))
		avatar = profileAvatarUrl(stored.entityHash)

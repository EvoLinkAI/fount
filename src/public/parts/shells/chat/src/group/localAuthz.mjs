 ...
 * 【原理】LOCAL_APPEND_AUTHZ_TYPES 白名单；reputation_slash/reset 校验目标活跃成员；peer_invite 校验 from/to 与可选 encrypted_H；已签名行跳过本地校验。
 * 【数据结构】事件 type/content、物化 state.members、LOCAL_APPEND_AUTHZ_TYPES Set。
 * 【关联】被 group/routes/dag.mjs 在批量追加前调用；依赖 access.mjs、chat/governance/blocklist。
 */
import { getState } from '../chat/dag/materialize.mjs'
import { isPubKeyHashBlocked } from '../chat/governance/blocklist.mjs'
 ...
 * @returns {void}
 */
export function validateLocalAuthzPayload(type, content, username, state) {
	if (!content || Array.isArray(content)) throw new Error('content object required')
	if (type === 'reputation_slash' || type === 'reputation_reset') {
		...
		const memberKey = resolveActiveMemberKey(state, username)
		const myHash = (memberKey && state.members[memberKey]?.pubKeyHash) || memberKey
		if (type === 'reputation_slash' && targetPubKeyHash === String(myHash || '').toLowerCase())
			throw new Error('cannot slash self')
		return
	}
	if (type === 'peer_invite') {
		const from = content.from?.trim() || username
		const to = content.to?.trim()
		if (from !== username) throw new Error('peer_invite from must match caller')
		if (!to || to === username) throw new Error('peer_invite requires distinct to')
		...
		if (content.encrypted_H !== undefined) {
			...
			if (!encryptedH?.ephemPub || !encryptedH?.iv || !encryptedH?.ciphertext || !encryptedH?.authTag)
...
export async function validateLocalAuthzBatch(username, groupId, events) {
	...
	for (const event of events) {
		if (!event || Array.isArray(event)) throw new Error('invalid event entry')
		...
		validateLocalAuthzPayload(eventType, event.content, username, state)

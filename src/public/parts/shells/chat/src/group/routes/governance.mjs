	...
	router.post(/^\/api\/parts\/shells:chat\/groups\/([^/]+)\/members\/([^/]+)\/(kick|ban|unban)$/, authenticate, async (req, res) => {
		...
		if (action === 'unban') {
			...
			await appendSignedLocalEvent(username, groupId, {
				type: 'member_unban',
				timestamp: Date.now(),
				content: { targetPubKeyHash: unbanPubKeyHash },
			})
		...
		if (action === 'ban') {
			...
			let banContent
			try {
				banContent = buildMemberBanContent(/** @type {import('../../chat/governance/banRules.mjs').BanScope} */ banScope, targetMember)
			}
			catch (error) {
	...
	router.post(/^\/api\/parts\/shells:chat\/groups\/([^/]+)\/owner-succession$/, authenticate, async (req, res) => {
			...
			for (const roleId of member.roles || [])
				if (state.roles[roleId]?.permissions?.MANAGE_ADMINS)
					await appendSignedLocalEvent(username, groupId, {
						type: 'role_revoke',
						timestamp: Date.now(),
						content: { targetPubKeyHash: hash, roleId },
					})
		...
		if (!(newOwnerMember?.roles || []).includes(transferRoleId))
			await appendSignedLocalEvent(username, groupId, {
				type: 'role_assign',
				timestamp: Date.now(),
				content: { targetPubKeyHash: targetHash, roleId: transferRoleId },
			})

...
/**
 * 【文件】channel/messageMutations.mjs
 * 【职责】频道消息编辑/删除/反馈的 DAG 追加：定位 JSONL 行 targetId，签发 message_edit/message_delete/message_feedback 事件。
 * 【原理】findChannelMessageRow 读 messagesPath JSONL；appendSignedLocalEvent 经 localSigner。物化后由 messageMerge 折叠展示。联邦出站走 index publishSignedEventToFederation。
...
export async function appendChannelMessageFeedback(username, groupId, channelId, eventId, type, reason) {
	if (type !== 'up' && type !== 'down') throw new Error('type must be up or down')
	const row = await findChannelMessageRow(username, groupId, channelId, eventId)
	if (!row) throw new Error('message not found')

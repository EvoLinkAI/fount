 ...
 * @param {{ fileId: string, data: Uint8Array, keyGeneration?: number, channelId?: string }} opts 明文与可选代数
...
export async function putEncryptedChunk(username, groupId, opts) {
	...
	const { contentHash, ciphertextHash, raw } = encryptConvergentPlaintext(opts.data)
 ...
 * @param {string} username 用户
 * @param {string} groupId 群
 * @param {{ fileId: string, data: Uint8Array, keyGeneration?: number }} opts 明文（用于推导 content/ciphertext 哈希）
...
export async function registerEncryptedChunkIfPresent(username, groupId, opts) {
	...
	const { contentHash, ciphertextHash } = encryptConvergentPlaintext(opts.data)
	const have = await hasCiphertextBlob(username, ciphertextHash)
	if (!have) return null
...
export async function getDecryptedFile(username, groupId, meta, blamePeerKey) {
	...
	if (parts?.length) {
		...
		for (const part of sorted) {
			const chunkHash = String(part?.ciphertextHash || ciphertextHashFromLocator(part?.storageLocator || '') || '')
				.trim()
				.toLowerCase()
			try {
	...
	return await getDecryptedChunk(username, groupId, meta.storageLocator, meta.contentHash, blamePeerKey)
}
 ...
 * @returns {Promise<Uint8Array>} 明文
 */
export async function getDecryptedChunk(username, groupId, storageLocator, contentHashHex, blamePeerKey) {
	...
	const plain = decryptConvergentCiphertext(raw, contentHash)
	if (!plain) {
		void penalizeChunkStorageFailure(username, groupId, blamePeerKey || username).catch(() => {})
...
export function registerGroupFileRoutes(router, authenticate, getUserByReq, getState, canInChannel, PERMISSIONS) {
	router.post(/^\/api\/parts\/shells:chat\/groups\/([^/]+)\/chunks\/have$/, authenticate, async (req, res) => {
		...
		let ciphertextHash = String(req.body?.ciphertextHash || '').trim().toLowerCase()
		if (!ciphertextHash && req.body?.data?.trim()) {
			const { data } = parseChunkBody({ fileId: 'probe', data: req.body.data })
			ciphertextHash = encryptConvergentPlaintext(data).ciphertextHash
		}
		if (!isHex64(ciphertextHash))
			return res.status(400).json({ error: 'ciphertextHash or data required' })
		...
		res.status(200).json({ ciphertextHash,
			have: have && sizeOk,
	...
	router.post(/^\/api\/parts\/shells:chat\/groups\/([^/]+)\/chunks$/, authenticate, async (req, res) => {
		...
		if (req.body?.registerOnly) {
			const registered = await registerEncryptedChunkIfPresent(username, groupId, { fileId, data })
			if (!registered)
				return res.status(404).json({ error: 'ciphertext not on this node' })
			return res.status(200).json({ ...registered })
		}

		res.status(200).json({ ...await putEncryptedChunk(username, groupId, {
			fileId,
			data,
	...
	router.get(/^\/api\/parts\/shells:chat\/groups\/([^/]+)\/chunks$/, authenticate, async (req, res) => {
		...
		const contentHash = String(req.query.content_hash || '')
		if (!storageLocator || !contentHash)
			return res.status(400).json({ error: 'locator and content_hash required' })

		const { state } = await getState(username, groupId)
		if (!await resolveActiveMemberKeyForLocalUser(username, groupId, state))
			return res.status(403).json({ error: 'Not a member' })

		const blamePeerKey = isHex64(req.query.blame_pub_key) ? String(req.query.blame_pub_key).trim().toLowerCase() : undefined
		const plain = await getDecryptedChunk(username, groupId, storageLocator, contentHash, blamePeerKey)
		res.status(200).json({ data: u8ToB64(plain) })
	})

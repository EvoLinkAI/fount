...
function normalizeRefcounts(raw) {
	...
	const src = o.refs && typeof o.refs === 'object' && !Array.isArray(o.refs) ? o.refs : o
	/** @type {Record<string, number>} */
	const out = {}
	for (const [k, v] of Object.entries(src)) {
		const h = String(k).trim().toLowerCase()
		const n = Number(v)
		if (isHex64(h) && Number.isFinite(n) && n > 0) out[h] = Math.floor(n)
	}
	return out
}

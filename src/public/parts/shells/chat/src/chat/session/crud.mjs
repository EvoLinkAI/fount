...
export async function newMetadata(groupId, username) {
	...
	if (defaults.LastTimeSlice.player_id)
		await appendSessionPersonaSet(username, groupId, defaults.LastTimeSlice.player_id)
	if (defaults.LastTimeSlice.world_id)
		await appendSessionWorldBind(username, groupId, defaults.LastTimeSlice.world_id)

 ...
 * 【原理】registerGroupUiSocket 在连接建立时清除 30 分钟卸载定时器并注册到 groupWsHub；onGroupWsClose 在房间无连接时启动定时器，到期根据 isVividGroup 仅清空 chatMetadata 或调用 deleteGroup 删盘；purgeGroupSession 同步中止流式与生成任务。

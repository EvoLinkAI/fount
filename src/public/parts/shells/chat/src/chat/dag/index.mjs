 ...
 * 【原理】联邦模块通过 `initFederationDagDeps` 解耦具体存储实现；此处绑定 `NODE_ID`、`readJsonl`、远程追加与 `getStateForFederation`，使 P2P 同步路径能复用同一套 DAG 管线。
 * 【数据结构】无本地状态；注入对象含 `nodeId`、函数句柄与 `getStateForFederation` 回调。

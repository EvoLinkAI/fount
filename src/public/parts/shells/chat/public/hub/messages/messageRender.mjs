...
export function isChannelMessageGenerating(message) {
	...
	if (message.is_generating === true) return true
...
export async function renderMessageContent(text) {
	...
	html = await replaceAsync(html, /\[sticker:([^\]]+)]/g, async () =>
		renderTemplateAsHtmlString('hub/messages/inline_sticker_placeholder', {}),
	)
...
async function renderStickerBlock(message) {
	...
	let src = content.stickerBase64 || content.stickerUrl || ''
...
export async function renderChannelMessageBlock(message, prevSender, prevTime, allMessages = [], renderOpts = {}) {
	...
	const sender = message.sender || message.charId || '?'

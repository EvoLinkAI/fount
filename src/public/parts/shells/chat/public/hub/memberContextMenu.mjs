 ...
 * 【原理】`showMemberContextMenu` 根据成员 DOM 节点定位菜单并展示角色/用户相关项。
 * 【数据结构】hubStore（core/state）及本模块函数入参/返回值；详见 JSDoc。
 * 【关联】发起私信可能导航到好友绑定群（由下游 `friendChat` 处理 hash）；../../../../scripts/i18n、../../../../scripts/template、../../../../scripts/toast、../src/api/groupApi、../src/groupViewerPermissions、banScopePicker、core/state、friendChat。
 */
...
export async function showMemberContextMenu(event, memberEl) {
	...
	const perms = viewer && viewer !== memberKey.toLowerCase()
		? await fetchViewerChannelPermissions(hubStore.currentState, hubStore.currentGroupId, defaultChannelId)
		: {}
	const showKick = !!perms.KICK_MEMBERS
	const showBan = !!perms.BAN_MEMBERS

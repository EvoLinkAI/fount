...
export async function init() {
	...
	function refreshStopBtn() {
		...
		if (sendBtn) sendBtn.toggleAttribute('hidden', active)
	}
	...
	document.getElementById('hub-stop-generation-button')?.addEventListener('click', () => {
		for (const id of getActiveVolatileStreamIds())
	...
	if (runUri) {
		...
		try {
			...
			showToastI18n('error', 'chat.hub.loadGroupFailed', { error: e?.message || String(e) })
		}
		if (applied?.groupId) {

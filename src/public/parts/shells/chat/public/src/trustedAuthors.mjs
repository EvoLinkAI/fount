...
const TRUSTED_AUTHORS_DB_VERSION = 2
...
function openTrustedAuthorsDatabase() {
	return new Promise((resolve, reject) => {
		const request = indexedDB.open(TRUSTED_AUTHORS_DB_NAME, TRUSTED_AUTHORS_DB_VERSION)
		/** @returns {void} */
		request.onerror = () => reject(request.error)
		/** @returns {void} */
		request.onupgradeneeded = () => {
			...
			if (database.objectStoreNames.contains(TRUSTED_AUTHORS_STORE))
				database.deleteObjectStore(TRUSTED_AUTHORS_STORE)

...
- **Chat 消息加密**: 频道域密钥 `K_ch`（`channel_key_rotate` DAG + HPKE wraps）；wire scheme **`ckg`**。**CKG 解密 payload 不可脱离外层 DAG Ed25519 签名上下文单独传递或信任**（对称层仅保密性）。群文件主密钥 `fileMasterKey`（`file_key_grant`）。密码学原语见 `scripts/p2p/key_crypto.mjs`。入站校验 `channel_key_rotate` 与 `assertFederatedCkgContent`。
- **Chat 帖子存储**: 热区（checkpoint `hot_posts` + `messages/*.jsonl`）+ 冷归档（`archive/{channelId}/{YYYY-MM}.jsonl` 明文 `PostSnapshot`）+ DAG 仅折叠过程事件；默认不自动删可见历史，管理员在群设置页按月经用户删冷归档。联邦按月拉取：`digest` + `fed_chunk_*` 分块传输 + `monthDigests` 信誉 digest 仲裁；冷归档 seal 含 `prevSealHash` 链。

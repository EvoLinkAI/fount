...
- **Chat 消息加密**: 频道域密钥 `K_ch`（`channel_key_rotate` DAG + HPKE wraps）；wire scheme **`ckg`**。群文件主密钥 `fileMasterKey`（`file_key_grant`）。入站校验 `channel_key_rotate` 与 `assertFederatedCkgContent`。
- **Chat 帖子存储**: 热区（checkpoint `hot_posts` + `messages/*.jsonl`）+ 冷归档（`archive/{channelId}/{YYYY-MM}.jsonl` 明文 `PostSnapshot`）+ DAG 仅折叠过程事件；默认不自动删可见历史，管理员在群设置页按月经用户删冷归档。联邦按月拉取验真：`monthDigests` + 多 peer 信誉加权 digest 仲裁（`archive/monthDigest.mjs`），非仅 owner seal。
